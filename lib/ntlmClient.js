...
function NtlmClient(credentials, AxiosConfig) {
    ...
    if (!config.httpAgent) {
        config.httpAgent = new agentkeepalive_1.default();
    }
    if (!config.httpsAgent) {
        config.httpsAgent = new agentkeepalive_1.default.HttpsAgent();
    }
    console.log(config);
        ...
        return __generator(this, function (_b) {
            switch (_b.label) {
                case 0:
                    error = err.response;
                    console.log(c++);
                    console.log(error === null || error === void 0 ? void 0 : error.headers, error === null || error === void 0 ? void 0 : error.status);
                    if (!(error &&
                        error.status === 401 &&
                        error.headers["www-authenticate"] &&
                        error.headers["www-authenticate"].includes("NTLM"))) return [3 /*break*/, 4];
